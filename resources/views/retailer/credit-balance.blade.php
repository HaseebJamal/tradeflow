...
<x-table><thead><tr><th>Date</th><th>Supplier</th><th>Description</th><th>Type</th><th>Amount</th><th>Balance</th></tr></thead><tbody>@forelse($ledgers ?? [] as $ledger)<tr><td>{{ $ledger->created_at->format('n/j/Y') }}</td><td>{{ $ledger->customer?->business?->business_name }}</td><td>{{ $ledger->description }}</td><td>{{ $ledger->type }}</td><td>Rs {{ number_format($ledger->amount) }}</td><td>Rs {{ number_format($ledger->balance) }}</td></tr>@empty<tr><td colspan="6" class="text-center tf-muted py-4">No credit records.</td></tr>@endforelse</tbody></x-table>
@endsection
