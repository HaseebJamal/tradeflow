...
<div class="tf-phone-input" data-tf-phone-field>
    <input id="{{ $inputId }}" type="tel" class="form-control {{ $error ? 'is-invalid' : '' }}" value="{{ $value }}" autocomplete="tel" inputmode="tel" placeholder="{{ $placeholder }}" data-tf-phone-visible data-default-country="{{ $defaultCountry }}" @required($required) @readonly($readonly) @disabled($disabled)>
