        ...
        <div class="col-lg-7">
            <form method="POST" action="{{ $action }}" class="row g-3" data-tf-confirm-message="These changes will update the footer shown on your business documents." data-tf-confirm-title="Save footer settings?" data-tf-confirm-button="Save Changes" data-tf-confirm-icon="question" data-tf-confirm-color="#2563eb" data-tf-confirm-saving-text="Saving...">
                ...
                @else
                    ...
                    <div class="col-12"><label class="form-label" for="footerAddress">Address</label><input id="footerAddress" name="address" class="form-control @error('address') is-invalid @enderror" maxlength="1000" value="{{ old('address', $business->address) }}">@error('address')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                    <div class="col-md-6"><label class="form-label" for="footerTaxNumber">NTN / Tax Number</label><input id="footerTaxNumber" name="tax_number" class="form-control @error('tax_number') is-invalid @enderror" maxlength="100" value="{{ old('tax_number', $business->tax_number) }}">@error('tax_number')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                    <div class="col-md-6"><label class="form-label">{{ $platformPoweredByText }}</label><input class="form-control" value="{{ $platformPoweredByText }}" readonly aria-readonly="true"><small class="tf-muted">This value follows the platform name.</small></div>
                @endif
                ...
                <div class="col-12"><h2 class="h6 mb-2">Show on Documents</h2><div class="row g-2">
                    @foreach(['show_company_name' => 'Business Name', 'show_footer_title' => 'Footer Title', 'show_footer_message' => 'Footer Message', 'show_phone' => 'Phone', 'show_email' => 'Business Email', 'show_address' => 'Address', 'show_website' => 'Website', 'show_tax_number' => 'NTN / Tax Number', 'show_powered_by' => $platformPoweredByText] as $field => $label)
