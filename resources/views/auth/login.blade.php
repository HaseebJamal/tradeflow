...
<section class="pp-login-shell">
    ...
    <div class="pp-login-panel"><div class="pp-login-card">
        ...
        <header><span class="pp-auth-eyebrow">WELCOME BACK</span><h1>Sign in </h1><p>Enter your details to continue to your business dashboard.</p></header>
        ...
        <form method="POST" action="{{ route('login.store') }}" data-tf-tab-order autocomplete="off">@csrf
            <div class="pp-auth-field"><label for="loginEmail">Email address</label><div class="pp-auth-input @error('email') is-invalid @enderror"><i class="bi bi-envelope" aria-hidden="true"></i><input id="loginEmail" name="email" type="email" placeholder="you@company.com" value="{{ old('email') }}" autocomplete="username" inputmode="email" autocapitalize="none" spellcheck="false" required aria-describedby="loginEmailError"></div>@error('email')<div id="loginEmailError" class="pp-auth-inline-error">{{ $message }}</div>@enderror</div>
