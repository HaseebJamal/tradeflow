...
<x-table><thead><tr><th>Product</th><th>Quantity</th><th>Unit cost</th><th>Line total</th></tr></thead><tbody>@foreach($return->items as $item)<tr><td>{{ $item->product?->name ?? $item->purchaseItem?->product_name_snapshot ?? '—' }}</td><td><x-quantity :value="$item->quantity" /></td><td>Rs {{ number_format($item->unit_cost, 2) }}</td><td>Rs {{ number_format($item->line_total, 2) }}</td></tr>@endforeach</tbody></x-table>
@endsection
