...
@else
    <form method="POST" action="{{ route('business.purchase-returns.start') }}" class="tf-card p-4">@csrf
        <div class="row g-3 align-items-end">
            <div class="col-md-8"><label class="form-label">Received purchase</label><select name="purchase_id" class="form-select" required autofocus><option value="">Select purchase</option>@foreach($purchases as $availablePurchase)<option value="{{ $availablePurchase->id }}">{{ $availablePurchase->purchase_number }} — {{ $availablePurchase->supplier?->supplier_name }} ({{ $availablePurchase->purchase_date?->format('n/j/Y') }})</option>@endforeach</select>@error('purchase_id')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
