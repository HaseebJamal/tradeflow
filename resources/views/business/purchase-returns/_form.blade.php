...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const quantityLabel = value => { const number = Number(value || 0); return Number.isFinite(number) ? String(Math.round(number * 1000) / 1000).replace(/\.?0+$/, '') || '0' : '0'; };
    const rows = () => [...body.querySelectorAll('[data-purchase-return-row]')];
    const selected = () => product.selectedOptions[0];
    const validateEntry = () => { const option = selected(), available = Number(option?.dataset.stock || 0), quantity = Number(qty.value || 0); qty.toggleAttribute('max', !!option?.value); if (option?.value) qty.max = String(available); const message = !option?.value ? '' : (quantity < 0.001 ? 'Quantity must be at least 0.001.' : (quantity > available ? `Return quantity cannot exceed available stock. Only ${quantityLabel(available)} units are available.` : '')); qty.classList.toggle('is-invalid', !!message); add.disabled = !option?.value || !!message; error.textContent = message; error.classList.toggle('d-none', !message); return !message; };
