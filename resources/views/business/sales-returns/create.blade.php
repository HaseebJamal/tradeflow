...
@section('page-subtitle', 'Select a completed POS or normal sale before choosing items to return')
@section('content')
        ...
        <div class="col-md-8">
            ...
            <select name="order_id" class="form-select" data-placeholder="Select completed sale" required autofocus>
                ...
                @foreach($orders as $order)
                    ...
                    <option value="{{ $order->id }}" @selected(old('order_id') == $order->id)>{{ $order->sale_channel === 'pos' ? 'POS' : 'SALE' }} - {{ $order->invoice?->invoice_number ?? $order->order_number }} - {{ $order->customer?->name ?? 'Walk-in Customer' }} - {{ $order->order_date?->timezone(config('app.timezone'))->format('d M Y h:i A') }} - Rs {{ number_format($order->grand_total, 2) }} - {{ $remainingItems }} returnable {{ \Illuminate\Support\Str::plural('item', $remainingItems) }}</option>
                @endforeach
            </select>
            @if($orders->isEmpty())<small class="text-warning d-block mt-2">No completed POS or normal sales are available for return.</small>@endif
