...
<form method="POST" action="{{ $editing ? route('business.staff.update', $staffMember) : route('business.staff.store') }}" enctype="multipart/form-data" class="row g-3" data-staff-form data-company-permission-form novalidate>
    ...
    <div class="col-md-3">
        ...
        <div class="tf-file-upload">
            <input id="staff-profile-image" name="profile_image" type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" class="visually-hidden @error('profile_image') is-invalid @enderror" data-tf-image-upload data-tf-file-upload>
            <label for="staff-profile-image" class="tf-file-upload__control"><i class="bi bi-cloud-arrow-up"></i><span><strong>Upload profile image</strong><small>JPG, PNG, or WebP up to 2 MB</small></span><em data-tf-file-upload-name>No file selected</em></label>
    ...
    <div class="col-md-3"><label for="staff-phone" class="form-label">Phone Number <span class="text-danger" aria-hidden="true">*</span></label><x-phone-input name="phone" id="staff-phone" :value="old('phone', $staffMember->phone ?? '')" :required="true" :error="$errors->first('phone')" /></div>
