...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-8"><div class="tf-card tf-expense-monthly-summary p-4"><h2 class="h6">Monthly Expense Summary</h2><div class="tf-expense-monthly-summary__items">@forelse($monthlySummary ?? [] as $summary)<div class="tf-expense-monthly-summary__item"><span>{{ \Illuminate\Support\Carbon::create($summary->year, $summary->month, 1)->format('F Y') }}</span><strong>Rs {{ number_format($summary->total, 2) }}</strong></div>@empty<span class="tf-muted">No monthly expense data yet.</span>@endforelse</div></div></div>
</div>

<x-table class="tf-business-data-table"><thead><tr><th>Date</th><th>Title</th><th>Category</th><th>Reason</th><th>Amount</th><th>Actions</th></tr></thead><tbody>@forelse($expenses as $expense)<tr><td>{{ ($expense->expense_date ?: $expense->date)?->format('n/j/Y') }}</td><td>{{ $expense->title }}</td><td>{{ $expense->category }}</td><td>{{ $expense->description ?: '-' }}</td><td>Rs {{ number_format($expense->amount, 2) }}</td><td>@companyCan('expenses.delete')<form method="POST" action="{{ route('business.expenses.destroy', $expense) }}">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger">Delete</button></form>@endcompanyCan</td></tr>@empty<tr><td colspan="6" class="text-center tf-muted py-4">No expenses found.</td></tr>@endforelse</tbody></x-table>
@if(method_exists($expenses, 'links'))<div class="mt-3"><x-table-result-summary :paginator="$expenses" />{{ $expenses->links('pagination::bootstrap-5') }}</div>@endif
@endsection
