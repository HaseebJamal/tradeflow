...
<section class="tf-card p-3 p-lg-4 mb-4">
    ...
    <form method="POST" action="{{ route('business.sales.payments.store') }}" enctype="multipart/form-data" class="row g-3 align-items-end" data-payment-form>@csrf
        ...
        <div class="col-sm-6 col-lg-2"><label class="form-label" for="payment-method">Method <span class="text-danger">*</span></label><select id="payment-method" name="method" class="form-select" required>@foreach(['Cash','Bank Transfer','JazzCash manual','Easypaisa manual','Cheque'] as $method)<option value="{{ $method }}" @selected(old('method', 'Cash') === $method)>{{ $method }}</option>@endforeach</select></div>
...
<form class="tf-card p-3 mb-3 row g-2 align-items-end">
    ...
    <div class="col-md-2"><label class="form-label">Method</label><select name="method" class="form-select"><option value="">All methods</option>@foreach(['Cash','Bank Transfer','JazzCash manual','Easypaisa manual','Cheque'] as $method)<option value="{{ $method }}" @selected(request('method') === $method)>{{ $method }}</option>@endforeach</select></div>
