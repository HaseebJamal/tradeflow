...
<div class="tf-card tf-suppliers-form-card p-4">
    <form method="POST" action="{{ route('business.suppliers.update', $supplier) }}" class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label">Opening Balance</label><div class="form-control bg-body-secondary">Rs {{ number_format($supplier->opening_balance ?? 0, 2) }}</div><small class="text-muted">Opening balances are posted once. Use Adjust Balance for corrections.</small></div>
