...
<form method="POST"
    ...
    <section class="mb-4">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-3"><label class="form-label">Supplier invoice number</label><input
                    name="supplier_invoice_number"
                    value="{{ old('supplier_invoice_number', $purchase?->supplier_invoice_number) }}"
                    class="form-control" placeholder="Generated automatically when saved" @readonly(!$isEditing)></div>
            ...
            <div class="col-md-3"><label class="form-label">Supplier reference</label><input name="supplier_reference"
                    value="{{ old('supplier_reference', $purchase?->supplier_reference) }}" class="form-control"></div>
            <div class="col-md-3"><label class="form-label">Purchase order reference</label><input
                    name="purchase_order_reference"
                    value="{{ old('purchase_order_reference', $purchase?->purchase_order_reference) }}"
    ...
    <section class="border rounded p-3 mb-3 bg-light" aria-label="Add purchase item">
        ...
        <div class="row g-2 align-items-end">
            ...
            <div class="col-xl-1"><label class="form-label">Quantity</label><input type="number" min="0" step="0.001"
                    value="0" class="form-control js-no-number-spinner js-no-wheel-change" data-purchase-entry-qty>
            </div>
            <div class="col-xl-2"><label class="form-label">Purchase cost</label><input type="number" min="0" step="any"
                    class="form-control js-no-number-spinner js-no-wheel-change" data-purchase-entry-cost></div>
            <div class="col-xl-2"><label class="form-label">Discount</label>
                <div class="input-group tf-adjustment-group"><select class="form-select tf-adjustment-type"
                    ...
                    </select><input type="number" min="0" step="any" value="0"
                        class="form-control js-no-number-spinner js-no-wheel-change" data-purchase-entry-discount-value>
                </div>
            </div>
            <div class="col-xl-2"><label class="form-label">Tax</label>
                <div class="input-group tf-adjustment-group"><select class="form-select tf-adjustment-type"
                        ...
                        class="form-control js-no-number-spinner js-no-wheel-change" data-purchase-entry-tax-value>
                </div>
            </div>
    ...
    <section class="row g-3 mt-1">
        <div class="col-lg-5"><label class="form-label">Other charges</label><input name="other_charges" type="number"
                min="0" step="any" value="{{ old('other_charges', $purchase?->other_charges ?? 0) }}"
                class="form-control" data-other-charges></div>
        <div class="col-lg-7 text-lg-end">
            <div class="pt-lg-4"><span class="me-3">Subtotal <strong data-purchase-subtotal>Rs 0.00</strong></span><span
        ...
        <div class="row g-3">
            <div class="col-md-4"><label class="form-label d-block">Payment type</label>
                <div class="btn-group w-100" role="group">
                    ...
                    for="paymentType{{ Str::slug($type) }}">{{ $type }}</label>@endforeach</div>
            </div>
            ...
            <div class="col-md-2"><label class="form-label">Amount paid now</label><input name="paid_amount"
                    type="number" min="0" step="any" value="{{ old('paid_amount', $purchase?->paid_amount ?? 0) }}"
                    class="form-control" data-paid-amount></div>
            ...
            <div class="col-md-2"><label class="form-label">Payment method</label><select name="payment_method"
            ...
            <div class="col-md-3"><label class="form-label">Payment date</label><input name="payment_date" type="date"
                    ...
                    class="form-control"></div>
            <div class="col-md-3"><label class="form-label">Reference number</label><input name="payment_reference"
                    value="{{ old('payment_reference', $purchase?->payment_reference) }}" class="form-control"></div>
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mt-4"><a
        ...
        <div class="d-flex gap-2"><button name="intent" value="draft" class="btn btn-outline-primary"
                data-purchase-submit>Save as Draft</button><button name="intent" value="confirm"
                class="btn btn-tf-primary" data-purchase-submit>Confirm Purchase</button></div>
    </div>
</form>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const error = $('[data-purchase-entry-error]'); const otherCharges = $('[data-other-charges]'); const paymentType = () => form.querySelector('[data-payment-type]:checked')?.value || 'Full Credit'; const paidAmount = $('[data-paid-amount]'); const paymentMethod = $('[data-payment-method]');
                const number = value => Math.max(0, Number.parseFloat(value || 0) || 0); const money = value => `Rs ${number(value).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`; const quantityLabel = value => { const amount = number(value); const normalized = String(Math.round(amount * 1000) / 1000); return normalized.includes('.') ? normalized.replace(/0+$/, '').replace(/\.$/, '') : normalized; }; const rows = () => [...body.querySelectorAll('[data-purchase-row]')];
                ...
                const add = () => { if (!supplier.value || supplier.value === '__create__') { setError('Please select or create a supplier before adding a purchase item.'); return; } const option = product.selectedOptions[0]; const amounts = lineAmounts(); if (!product.value || number(qty.value) < 0.001 || number(cost.value) < 0 || discountType.value === 'percentage' && number(discountValue.value) > 100 || taxType.value === 'percentage' && number(taxValue.value) > 100 || amounts.discount > amounts.subtotal) { setError('Select a product and enter valid quantity, cost, discount and tax values.'); return; } const existing = rows().find(row => row.dataset.productId === product.value); const target = editing || existing || document.createElement('tr'); const existingQty = existing && existing !== editing ? number(existing.querySelector('[name$="[quantity]"]').value) : 0; writeRow(target, { product_id: product.value, product_name: option.text, unit: option.dataset.unit || 'Unit', quantity: quantityLabel(existingQty + number(qty.value)), unit_cost: number(cost.value).toFixed(2), discount_type: discountType.value, discount_value: number(discountValue.value).toFixed(2), tax_type: taxType.value, tax_value: number(taxValue.value).toFixed(2) }); if (!editing && !existing) body.appendChild(target); render(); reset(); };
