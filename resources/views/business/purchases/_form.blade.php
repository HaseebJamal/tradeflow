@php
    ...
    ])->values()->all() ?? ($purchasePrefillItems ?? []));
...
<form method="POST"
    ...
    <section class="border rounded p-3 mb-3 bg-light" aria-label="Add purchase item">
        ...
        <div class="row g-2 align-items-end">
            ...
            <div class="col-xl-1"><label class="form-label">Qty</label><input type="number" min="0" step="1"
                    value="0" class="form-control js-whole-number js-no-number-spinner js-no-wheel-change" data-purchase-entry-qty>
            </div>
            <div class="col-xl-1"><label class="form-label">Free Qty</label><input type="number" min="0" step="1"
                    value="0" class="form-control js-whole-number js-no-number-spinner js-no-wheel-change" data-purchase-entry-free-qty></div>
        ...
        <div class="form-text">Bonus quantity supplied at no additional purchase cost.</div>
        ...
        <table class="table align-middle mb-0">
            ...
            <tbody data-purchase-items>
                <tr data-purchase-empty>
                    <td colspan="11" class="text-center tf-muted py-4">No purchase items added yet.</td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    <section class="border rounded p-3 mt-4" aria-label="Payment details">
        ...
        <div class="row g-3">
            ...
            <select name="payment_method"
                    ...
                    @foreach(['Cash', 'Bank Transfer', 'Jazz Cash', 'Easypaisa', 'Cheque', 'Other'] as $method)
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const stock = $('[data-purchase-entry-stock]'); const qty = $('[data-purchase-entry-qty]'); const freeQty = $('[data-purchase-entry-free-qty]'); const cost = $('[data-purchase-entry-cost]');
                ...
                const rawMoney = value => (window.tradeFlowRawMoney?.(value) ?? String(value ?? '').replace(/,/g, '')).replace(/^\s*Rs\.?\s*/i, '').trim();
                const number = value => {
                    const raw = rawMoney(value);
                    return /^\d+(?:\.\d+)?$/.test(raw) ? Math.max(0, Number(raw)) : 0;
                };
                const whole = value => String(Math.round(number(value)));
                const money = value => { const amount = number(value); return `Rs ${amount.toLocaleString(undefined, { minimumFractionDigits: Number.isInteger(amount) ? 0 : 2, maximumFractionDigits: 2 })}`; };
                const quantityLabel = value => whole(value); const rows = () => [...body.querySelectorAll('[data-purchase-row]')];
                ...
                const updateAvailability = () => { const ready = Boolean(supplier.value) && supplier.value !== '__create__';[product, qty, freeQty, cost, discountType, discountValue, taxType, taxValue, $('[data-add-purchase-item]')].forEach(input => { if (input) input.disabled = !ready; }); const control = window.getTradeFlowTomSelect?.(product); ready ? control?.enable() : control?.disable(); };
                ...
                const writeRow = (row, item) => { item = { ...item, quantity: whole(item.quantity), free_quantity: whole(item.free_quantity), unit_cost: whole(item.unit_cost), discount_value: whole(item.discount_value), tax_value: whole(item.tax_value) }; const amounts = lineAmounts(item); const itemQuantity = quantityLabel(item.quantity); const itemFreeQuantity = quantityLabel(item.free_quantity); row.dataset.purchaseRow = ''; row.dataset.productId = item.product_id; row.dataset.total = amounts.total; row.dataset.subtotal = amounts.subtotal; row.dataset.discount = amounts.discount; row.dataset.tax = amounts.tax; row.innerHTML = `<td data-index></td><td>${escapeHtml(item.product_name)}<input type="hidden" name="items[0][product_id]" value="${item.product_id}"></td><td>${itemQuantity}<input type="hidden" name="items[0][quantity]" value="${itemQuantity}"></td><td>${itemFreeQuantity}<input type="hidden" name="items[0][free_quantity]" value="${itemFreeQuantity}"></td><td>${escapeHtml(item.unit || 'Unit')}</td><td>${money(item.unit_cost)}<input type="hidden" name="items[0][unit_cost]" value="${item.unit_cost}"></td><td>${adjustment(item.discount_type, item.discount_value)}<input type="hidden" name="items[0][discount_type]" value="${item.discount_type}"><input type="hidden" name="items[0][discount_value]" value="${item.discount_value}"></td><td>${adjustment(item.tax_type, item.tax_value)}<input type="hidden" name="items[0][tax_type]" value="${item.tax_type}"><input type="hidden" name="items[0][tax_value]" value="${item.tax_value}"></td><td>${money(amounts.total)}</td><td><button type="button" class="btn btn-sm btn-outline-primary" data-edit-purchase-item>Edit</button></td><td><button type="button" class="btn btn-sm btn-outline-danger" data-delete-purchase-item>Delete</button></td>`; };
                ...
                const reset = () => { editing = null; const productControl = window.getTradeFlowTomSelect?.(product); if (productControl) { productControl.clear(true); productControl.close(); } else { product.value = ''; } qty.value = '0'; freeQty.value = '0'; cost.value = ''; discountType.value = 'percentage'; discountValue.value = '0'; taxType.value = 'percentage'; taxValue.value = '0'; stock.value = ''; clearError(); refreshOptions(); };
                ...
                const add = () => { if (!supplier.value || supplier.value === '__create__') { setError('Please select or create a supplier before adding a purchase item.'); return; } const option = product.selectedOptions[0]; const amounts = lineAmounts(); const editableValues = [qty, freeQty, cost, discountValue, taxValue]; if (editableValues.some(input => !/^\d+$/.test(String(input.value).replace(/,/g, '')))) { setError('Only whole numbers are allowed.'); return; } if (!product.value || number(qty.value) < 1 || number(freeQty.value) < 0 || number(cost.value) < 0 || discountType.value === 'percentage' && number(discountValue.value) > 100 || taxType.value === 'percentage' && number(taxValue.value) > 100 || amounts.discount > amounts.subtotal) { setError('Select a product and enter valid quantity, bonus quantity, cost, discount and tax values.'); return; } const existing = rows().find(row => row.dataset.productId === product.value); const target = editing || existing || document.createElement('tr'); const existingQty = existing && existing !== editing ? number(existing.querySelector('[name$="[quantity]"]').value) : 0; const existingFreeQty = existing && existing !== editing ? number(existing.querySelector('[name$="[free_quantity]"]').value) : 0; writeRow(target, { product_id: product.value, product_name: option.text, unit: option.dataset.unit || 'Unit', quantity: whole(existingQty + number(qty.value)), free_quantity: whole(existingFreeQty + number(freeQty.value)), unit_cost: whole(cost.value), discount_type: discountType.value, discount_value: whole(discountValue.value), tax_type: taxType.value, tax_value: whole(taxValue.value) }); if (!editing && !existing) body.appendChild(target); render(); reset(); };
