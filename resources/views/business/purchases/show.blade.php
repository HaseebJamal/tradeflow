...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4"><a href="{{ route('business.purchases.index') }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Purchases</a><div class="d-flex gap-2"><span class="tf-badge {{ in_array($purchase->status, ['Confirmed','Received','Closed'], true) ? 'tf-badge-success' : ($purchase->status === 'Cancelled' ? 'tf-badge-danger' : 'tf-badge-warning') }}">{{ $purchase->status }}</span><span class="tf-badge {{ $paymentSummary['payment_status'] === 'Paid' ? 'tf-badge-success' : 'tf-badge-warning' }}">{{ $paymentSummary['payment_status'] }}</span><span class="tf-badge {{ $receiptState['receipt_status'] === 'Fully Received' ? 'tf-badge-success' : 'tf-badge-warning' }}">{{ $receiptState['receipt_status'] }}</span></div></div>
