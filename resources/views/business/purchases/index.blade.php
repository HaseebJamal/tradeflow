...
<x-table><thead><tr><th>Purchase</th><th>Supplier</th><th>Date</th><th>Total Qty</th><th>Grand total</th><th>Paid / Payable</th><th>Payment</th><th>Purchase status</th><th>Receiving</th><th>Created by</th><th></th></tr></thead><tbody>@forelse($purchases as $purchase)<tr><td><strong>{{ $purchase->purchase_number }}</strong><small class="d-block tf-muted">{{ $purchase->supplier_invoice_number ?: 'No supplier invoice' }}</small></td><td>{{ $purchase->supplier?->supplier_name }}</td><td><x-date-time :value="$purchase->purchase_date" /></td><td><x-quantity :value="$purchase->items_sum_quantity" /></td><td>Rs {{ number_format($purchase->grand_total, 2) }}</td><td>Rs {{ number_format($purchase->paid_amount, 2) }}<small class="d-block tf-muted">Payable Rs {{ number_format($purchase->balance, 2) }}</small></td><td>{{ $purchase->payment_method ?: 'Not paid' }}<small class="d-block"><span class="tf-badge {{ $purchase->payment_status === 'Paid' ? 'tf-badge-success' : 'tf-badge-warning' }}">{{ $purchase->payment_status }}</span></small></td><td><span class="tf-badge {{ in_array($purchase->status, ['Confirmed','Received','Closed'], true) ? 'tf-badge-success' : ($purchase->status === 'Cancelled' ? 'tf-badge-danger' : 'tf-badge-warning') }}">{{ $purchase->status }}</span></td><td><span class="tf-badge {{ $purchase->receiving_status === 'Fully Received' ? 'tf-badge-success' : 'tf-badge-warning' }}">{{ $purchase->receiving_status ?? 'Not Received' }}</span></td><td>{{ $purchase->creator?->name ?? 'System' }}</td><td><a href="{{ route('business.purchases.show', $purchase) }}" class="btn btn-sm btn-outline-primary">Manage</a></td></tr>@empty<tr><td colspan="11" class="text-center tf-muted py-5">No purchases found.</td></tr>@endforelse</tbody></x-table><div class="mt-3">{{ $purchases->links() }}</div>
@endsection
