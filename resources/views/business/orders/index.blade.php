...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-3">
    ...
    <div class="d-flex flex-wrap gap-2">
        ...
        @companyCan('sales.create')
            <a href="{{ route('business.sales.create') }}" class="btn btn-tf-primary"><i class="bi bi-plus-lg me-1"></i>Create Sale</a>
        @endcompanyCan
    </div>
</div>
<form class="tf-card p-4 mb-3" data-code-lookup-form data-code-lookup-url="{{ route('business.sales.lookup') }}">
    <div class="row g-2 align-items-end">
        ...
        <div class="col-md-2"><label class="form-label">Status</label><select name="status" class="form-select"><option value="">All</option>@foreach(['New','Accepted','Packing','Ready','Out For Delivery','Delivered','Completed','Returned','Cancelled','Void'] as $status)<option @selected(request('status')===$status)>{{ $status }}</option>@endforeach</select></div>
...
<x-table>
    <thead><tr><th>Sale Number</th><th>Customer</th><th>Product Summary</th><th>Status</th><th>Payment Status</th><th>Payment Type</th><th>Created By</th><th>Sale Date and Time</th><th>Updated At</th><th>Total</th><th>Actions</th></tr></thead>
    <tbody>
    ...
    @empty
        <tr><td colspan="11" class="text-center tf-muted py-4">No sales yet.</td></tr>
    @endforelse
    </tbody>
</x-table>
