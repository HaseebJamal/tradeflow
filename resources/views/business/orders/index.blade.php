...
<form class="tf-card p-4 mb-3" data-code-lookup-form data-code-lookup-url="{{ route('business.sales.lookup') }}">
    <div class="row g-2 align-items-end">
        ...
        <div class="col-md-2"><label class="form-label">Payment Type</label><select name="payment_type" class="form-select"><option value="">All</option>@foreach(['Cash','Credit','Split','Card','Bank Transfer','Jazz Cash','Easypaisa','Cheque','Other','Partial'] as $type)<option @selected(request('payment_type')===$type)>{{ $type }}</option>@endforeach</select></div>
