...
<body class="p-4">
    ...
    <table class="table table-bordered"><thead><tr><th>Product</th><th>Qty</th><th>Rate</th><th>Total</th></tr></thead><tbody>@foreach($delivery->sourceOrder()?->items ?? [] as $item)<tr><td>{{ $item->product_name_snapshot ?: $item->product?->name }}</td><td>{{ $item->quantity }} {{ $item->unit }}</td><td>Rs {{ number_format($item->unit_price ?: $item->price) }}</td><td>Rs {{ number_format($item->line_total ?: $item->total) }}</td></tr>@endforeach</tbody></table>
