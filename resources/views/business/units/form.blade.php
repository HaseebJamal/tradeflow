...
<div class="tf-card p-4" style="max-width:760px">
    <form method="POST" action="{{ isset($unit) ? route('business.units.update', $unit) : route('business.units.store') }}" class="row g-3">
        ...
        <div class="col-12"><label class="form-label">Unit Name <span class="text-danger">*</span></label><input name="unit_name" class="form-control @error('unit_name') is-invalid @enderror" value="{{ old('unit_name', $unit->unit_name ?? '') }}" required autofocus>@error('unit_name')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
        <div class="col-md-6"><label class="form-label">Unit Type <span class="text-danger">*</span></label><select name="unit_type" class="form-select @error('unit_type') is-invalid @enderror" required><option value="">Select type</option>@foreach(['Piece','Weight','Volume','Length','Area','Pack','Other'] as $type)<option value="{{ $type }}" @selected(old('unit_type', $unit->unit_type ?? '') === $type) @disabled(in_array($type, $usedUnitTypes ?? [], true))>{{ $type }}@if(in_array($type, $usedUnitTypes ?? [], true)) (Already created)@endif</option>@endforeach</select>@error('unit_type')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
