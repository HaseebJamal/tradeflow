...
<div class="tf-card p-4" style="max-width:760px">
    <form method="POST" action="{{ isset($unit) ? route('business.units.update', $unit) : route('business.units.store') }}" class="row g-3">
        ...
        <div class="col-12"><label class="form-label">Unit Name <span class="text-danger">*</span></label><div class="tf-identity-input-wrap"><input name="unit_name" class="form-control tf-identity-input @error('unit_name') is-invalid @enderror @error('unit_name_normalized') is-invalid @enderror" value="{{ isset($unit) ? $unit->unit_name : old('unit_name', '') }}" @readonly(isset($unit)) required autofocus>@isset($unit)<i class="bi bi-lock-fill" aria-hidden="true"></i>@endisset</div>@isset($unit)<div class="form-text tf-identity-helper">Unit name cannot be changed after creation.</div>@endisset @error('unit_name')<div class="invalid-feedback">{{ $message }}</div>@enderror @error('unit_name_normalized')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
