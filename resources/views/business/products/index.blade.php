...
<form class="tf-card p-4 mb-3" data-code-lookup-form data-code-lookup-url="{{ route('business.products.lookup') }}">
    <div class="row g-2 align-items-end">
        <div class="col-md-3"><label class="form-label">Search / Barcode</label><input id="productBarcodeSearch" name="search" class="form-control" value="{{ request('search') }}" placeholder="Name, SKU, barcode" autocomplete="off" autofocus data-code-lookup></div>
...
<x-table>
    ...
    @forelse($products ?? [] as $product)
        <tr>
            ...
            <td>{{ $product->unit }}</td>
            <td>Rs {{ number_format($product->purchase_cost, 2) }}</td>
