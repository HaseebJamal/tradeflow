    ...
    <div class="col-xl-10">
        <div class="tf-card p-4 p-lg-5 mb-4 text-center">
            ...
            <div class="row g-3 text-start">
                ...
                <div class="col-md-3"><div class="border rounded p-3 h-100"><small class="tf-muted d-block">Customer</small><strong>{{ $order->customer?->business_name ?: $order->customer?->name ?: 'Walk-in Customer' }}</strong></div></div>
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-lg-6">
                <div class="tf-card p-4 h-100">
                    ...
                    @companyCan('invoices.view')<div class="d-flex flex-wrap gap-2"><a href="{{ route('business.invoices.show', $order) }}" class="btn btn-tf-navy"><i class="bi bi-file-earmark-check me-1"></i>Generate invoice</a>@companyCan('invoices.export')<a href="{{ route('business.invoices.pdf', $order) }}" target="_blank" rel="noopener" class="btn btn-outline-primary">Invoice PDF</a>@endcompanyCan</div>@else<p class="small text-muted mb-0">You do not have invoice-viewing permission.</p>@endcompanyCan
                    </div></div>
                </div>
            </div>
        </div>
        ...
        <div class="tf-card p-3 p-lg-4">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3"><div><h2 class="h5 mb-1">Manage this sale</h2><p class="tf-muted small mb-0">Continue working without losing the saved transaction.</p></div><div class="d-flex flex-wrap gap-2"><a href="{{ route('business.pos.index') }}" class="btn btn-tf-primary"><i class="bi bi-plus-lg me-1"></i>New sale</a><a href="{{ route('business.pos.history') }}" class="btn btn-outline-primary"><i class="bi bi-clock-history me-1"></i>Sales history</a>@companyCan('pos.returns')<a href="{{ route('business.pos.returns', $order) }}" class="btn btn-outline-warning"><i class="bi bi-arrow-return-left me-1"></i>Process return</a>@endcompanyCan</div></div>
        </div>
    </div>
</div>
@endsection
