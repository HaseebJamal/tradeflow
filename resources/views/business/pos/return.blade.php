...
<form method="POST" action="{{ route(request()->routeIs('business.sales.returns.*') ? 'business.sales.returns.store' : 'business.pos.returns.store', $order) }}" class="tf-card p-4" data-pos-return-form>
    ...
    <section class="border rounded bg-light p-3 mb-3" aria-label="Add return item">
        ...
        <div class="row g-2 align-items-end">
            <div class="col-lg-3"><label class="form-label">Product</label><select class="form-select" data-pos-return-product><option value="">Select product</option>@foreach($order->items as $item)@php($returned = $item->posReturnItems->sum('quantity'))@php($returnable = $item->quantity - $returned)@php($effectiveRefundPrice = $item->quantity ? (($item->line_total ?? $item->total ?? 0) / $item->quantity) : 0)@continue($returnable < 1)<option value="{{ $item->id }}" data-name="{{ $item->product_name_snapshot ?: $item->product?->name }}" data-stock="{{ $returnable }}" data-unit="{{ $item->unit ?: $item->product?->unit }}" data-price="{{ $effectiveRefundPrice }}">{{ $item->product_name_snapshot ?: $item->product?->name }}</option>@endforeach</select></div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sync = () => { const selected = option(); stock.value = selected?.value ? `${selected.dataset.stock} ${selected.dataset.unit || ''}` : ''; price.value = selected?.value ? selected.dataset.price || 0 : ''; total.value = money((Number(qty.value) || 0) * Number(price.value || 0)); validateEntry(); };
