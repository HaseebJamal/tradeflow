    ...
    <tbody>
        @forelse($tickets as $ticket)
            <tr>
                ...
                <td><div class="d-flex align-items-center gap-1"><button type="button" class="btn btn-sm btn-tf-primary" data-bs-toggle="modal" data-bs-target="#ticketManageModal-{{ $ticket->id }}">Manage</button><div class="dropdown"><button class="btn btn-sm btn-outline-secondary" data-bs-toggle="dropdown" aria-label="Ticket actions"><i class="bi bi-three-dots"></i></button><ul class="dropdown-menu dropdown-menu-end"><li><button type="button" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#ticketManageModal-{{ $ticket->id }}">View details</button></li><li><button type="button" class="dropdown-item" data-ticket-modal-focus="conversation" data-bs-toggle="modal" data-bs-target="#ticketManageModal-{{ $ticket->id }}">View history</button></li>@if($ticket->status !== 'Closed')<li><button type="button" class="dropdown-item" data-ticket-set-status="Closed" data-bs-toggle="modal" data-bs-target="#ticketManageModal-{{ $ticket->id }}">Close ticket</button></li>@endif</ul></div></div></td>
            </tr>
        @empty
...
@foreach($tickets as $ticket)
    <div class="modal fade tf-ticket-modal" id="ticketManageModal-{{ $ticket->id }}" tabindex="-1" aria-labelledby="ticketManageTitle-{{ $ticket->id }}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <form method="POST" action="{{ route('admin.support-tickets.update', $ticket) }}" class="modal-content" data-ticket-manage-form>
                @csrf @method('PATCH')
                ...
                <div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Close</button><div class="d-flex flex-wrap gap-2 ms-auto"><button type="submit" name="action" value="save" class="btn btn-outline-primary">Save changes</button><button type="submit" name="action" value="reply" class="btn btn-tf-primary" data-ticket-send-reply>Send reply</button></div></div>
            </form>
        </div>
    </div>
@endforeach
...
document.addEventListener('click', (event) => {
    ...
    if (action) document.querySelector(`${action.dataset.bsTarget} [data-ticket-status]`).value = action.dataset.ticketSetStatus;
