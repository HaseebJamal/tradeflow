...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-3">
    ...
    <div class="d-flex gap-2"><button type="button" class="btn btn-outline-secondary" data-live-toggle>Pause Live Updates</button><a class="btn btn-outline-primary" href="{{ route('admin.audit-logs.export.csv', request()->query()) }}"><i class="bi bi-filetype-csv me-1"></i>CSV</a><a class="btn btn-outline-primary" href="{{ route('admin.audit-logs.export.pdf', request()->query()) }}" target="_blank" rel="noopener noreferrer"><i class="bi bi-filetype-pdf me-1"></i>PDF</a></div>
</div>
<div class="tf-card p-3 mb-3"><form method="GET" class="row g-2 align-items-end">
    ...
    <div class="col-md-3"><label class="form-label">Search</label><input name="search" value="{{ request('search') }}" class="form-control" placeholder="Description, route, or action"></div>
...
<x-table><thead><tr><th>Date &amp; Time</th><th>Company</th><th>User</th><th>Role</th><th>Module</th><th>Action</th><th>Description</th><th>IP Address</th><th class="text-end">Details</th></tr></thead><tbody data-audit-log-body>
@forelse($logs as $log)
    ...
    <tr data-audit-log-id="{{ $log->id }}"><td><x-date-time :value="$log->occurred_at ?? $log->created_at" /></td><td>{{ $log->business?->business_name ?: 'Platform' }}</td><td>{{ $log->user_name ?: $log->user?->name ?: 'System' }}</td><td>{{ $log->role ?: $log->actor_role ?: 'system' }}</td><td>{{ $log->module ?: 'General' }}</td><td>{{ $log->action }}</td><td>{{ $log->description ?: $log->action }}</td><td>{{ $log->ip_address ?: '—' }}</td><td class="text-end"><button type="button" class="btn btn-sm btn-outline-primary" data-audit-details="{{ $detailPayload }}">View</button></td></tr>
@empty
    <tr data-audit-empty><td colspan="9" class="text-center tf-muted py-4">No audit activity has been recorded yet.</td></tr>
@endforelse
</tbody></x-table>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const row = (log) => '<tr data-audit-log-id="' + log.id + '"><td>' + esc(log.occurred_at) + '</td><td>' + esc(log.company) + '</td><td>' + esc(log.user) + '</td><td>' + esc(log.role) + '</td><td>' + esc(log.module) + '</td><td>' + esc(log.action) + '</td><td>' + esc(log.description) + '</td><td>' + esc(log.ip_address) + '</td><td class="text-end"><button type="button" class="btn btn-sm btn-outline-primary" data-live-detail="' + encodeURIComponent(JSON.stringify(log)) + '">View</button></td></tr>';
