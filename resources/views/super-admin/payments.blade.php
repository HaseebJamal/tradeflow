...
<div class="tf-billing-workspace">
            ...
            <form method="POST" action="{{ route('admin.payments.store') }}" class="tf-billing-payment-form">
                ...
                <fieldset>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-xl-2 col-md-3"><label class="form-label">Payment Method</label><select name="method" class="form-select" required>@foreach(['Cash','Bank Transfer','Jazz Cash','Easypaisa','Cheque','Other'] as $method)<option value="{{ $method }}" @selected(old('method', $recordRenewal?->last_payment_method) === $method)>{{ $method }}</option>@endforeach</select></div>
    ...
    @else
    <section class="tf-card tf-billing-table-card">
        <div class="p-3 border-bottom"><h2 class="h5 mb-1">Renewal Invoices</h2><p class="tf-muted small mb-0">Automatic reminders for real custom paid-access cycles. Suggested amounts come from the latest agreed payment and remain editable before recording payment.</p></div>
        <div class="table-responsive tf-dropdown-safe-scroll">
            <table class="table tf-admin-data-table mb-0 tf-has-actions-column">
