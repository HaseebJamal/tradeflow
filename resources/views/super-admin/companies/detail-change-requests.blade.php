...
@forelse($requests as $changeRequest)
    <div class="tf-card p-4 mb-3">
        ...
        @if($changeRequest->status === 'Pending')
            <div class="row g-2">
                <form method="POST" action="{{ route('admin.business-detail-change-requests.approve', $changeRequest) }}" class="col-md-6" data-tf-confirm-message="Approve this request for application? This step does not change business details or notify the owner.">@csrf @method('PATCH')<label class="form-label">Approval Note <span class="tf-muted small">Optional</span></label><div class="input-group"><input name="review_note" class="form-control" maxlength="2000" placeholder="Optional review note"><button class="btn btn-success">Approve Request</button></div></form>
