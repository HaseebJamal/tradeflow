...
<x-table class="tf-admin-data-table">
    <thead><tr><th>User</th><th>Role</th><th>Business / Scope</th><th>Last sign-in</th><th>Created</th><th>Status</th><th class="text-end">Actions</th></tr></thead>
    <tbody>
    @forelse($users as $user)
        @php($statusClass = match($user->status) { 'active' => 'success', 'suspended' => 'danger', default => 'warning' })
        <tr>
            <td><div class="d-flex align-items-center gap-2"><span class="tf-icon-tile bg-blue text-white" style="width:34px;height:34px"><i class="bi bi-person"></i></span><div><strong class="d-block">{{ $user->name }}</strong><small class="d-block tf-muted">{{ $user->email }}</small>@if($user->phone)<small class="d-block tf-muted">{{ $user->phone }}</small>@endif</div></div></td>
            ...
            <td><small>{{ $user->last_login_at?->format('n/j/Y, g:i A') ?? 'Never recorded' }}</small></td>
            <td><small>{{ $user->created_at?->format('n/j/Y, g:i A') ?? '—' }}</small></td>
            ...
            <td class="text-end"><div class="btn-group">
                ...
                @elseif(!$user->business_id)
                    <form method="POST" action="{{ route('admin.users.status', $user) }}" data-tf-confirm-message="{{ $user->status === 'active' ? 'This platform account will be suspended until it is reactivated.' : 'This platform account will become active again.' }}" data-tf-confirm-title="{{ $user->status === 'active' ? 'Suspend platform account?' : 'Activate platform account?' }}" data-tf-confirm-button="{{ $user->status === 'active' ? 'Suspend' : 'Activate' }}" data-tf-confirm-color="{{ $user->status === 'active' ? '#dc3545' : '#2563eb' }}">@csrf @method('PATCH')<input type="hidden" name="status" value="{{ $user->status === 'active' ? 'suspended' : 'active' }}"><button class="btn btn-sm btn-outline-{{ $user->status === 'active' ? 'danger' : 'success' }}">{{ $user->status === 'active' ? 'Suspend' : 'Activate' }}</button></form>
