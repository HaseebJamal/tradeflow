...
<div class="tf-card p-3 p-lg-4 mb-3">
    <div class="d-flex flex-wrap justify-content-between gap-2 mb-3">
        <div><h2 class="h5 mb-1">Account directory</h2><p class="tf-muted small mb-0">Filter accounts, control access, and reset credentials securely.</p></div>
        <span class="tf-badge tf-badge-info">{{ $users->total() }} matching {{ Str::plural('account', $users->total()) }}</span>
    </div>
    <form method="GET" class="row g-3 align-items-end">
        <div class="col-lg-4"><label for="user-search" class="form-label">Search</label><input id="user-search" name="search" value="{{ request('search') }}" class="form-control" placeholder="Name, email, or phone" autofocus></div>
...
<x-table>
    ...
    @forelse($users as $user)
        ...
        <tr>
            <td><div class="d-flex align-items-center gap-2"><span class="tf-icon-tile bg-blue text-white" style="width:34px;height:34px"><i class="bi bi-person"></i></span><div><strong class="d-block">{{ $user->name }}</strong><small class="tf-muted">{{ $user->email }}</small></div></div></td>
