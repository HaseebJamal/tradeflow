...
<x-table>
<thead><tr><th>Time</th><th>User</th><th>Role</th><th>Business</th><th>Module</th><th>Action</th><th>Description</th><th>IP Address</th><th>Device/Browser</th></tr></thead>
<tbody>@forelse($activities as $activity)<tr><td><x-date-time :value="$activity->occurred_at" /></td><td>{{ $activity->actor_name_snapshot ?: $activity->actor?->name }}</td><td>{{ $activity->actor_role }}</td><td>{{ $activity->business?->business_name ?? '-' }}</td><td>{{ $activity->module }}</td><td>{{ $activity->action }}</td><td>{{ $activity->description }}</td><td>{{ $activity->ip_address }}</td><td class="text-truncate" style="max-width:220px">{{ $activity->user_agent }}</td></tr>@empty<tr><td colspan="9" class="text-center tf-muted py-4">No activity recorded.</td></tr>@endforelse</tbody>
</x-table>
